            ...
            >
              {current ? (
                current.media_type === 'video' ? (
                  ...
                  <img src={current.url} alt={p.name} className="h-full w-full object-cover" />
                )
              ) : (
              ...
              <div className="mt-3 flex gap-2 overflow-x-auto">
                {media.map((m, i) => (
                  <button key={m.id} onClick={() => setIdx(i)}
                    className={`h-16 w-16 flex-shrink-0 overflow-hidden border-2 ${i === idx ? 'border-ink' : 'border-transparent'}`}>
                    {m.media_type === 'video' ? (
